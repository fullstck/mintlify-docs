...
<ParamField query="advertPriceMax" type="integer">
  Maximum price of advert. Filters based on any non expired adverts prices.
</ParamField>

<ParamField query="advertPriceMin" type="integer">
  Minimum price of advert. Filters based on any non expired adverts prices.
</ParamField>
...
<ParamField query="geoAccuracy" type="array">
  Filters properties by geocoding confidence level. Use `1` to keep only house-number level matches and `2` to include neighborhood-level approximations.
</ParamField>
...
<ParamField query="withLocation" type="boolean">
  Restrict results to properties that have a resolved location.
</ParamField>
