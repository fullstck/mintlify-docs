...
<ParamField query="order[updatedAt]" type="string">
  Order properties by update date (API parameter name includes the double “d”). Can be `asc` or `desc`.
</ParamField>
...
<ParamField query="withCoherentPrice" type="boolean">
  Price coherence filter. `true` returns only properties whose price passes our coherence checks; `false` returns only properties flagged as incoherent. Omit the parameter to get all properties regardless of price coherence. Example: `withCoherentPrice=true`
</ParamField>
...
headers = {
  ...
  'X-API-KEY': process.env.STREAMESTATE_API_KEY
}
