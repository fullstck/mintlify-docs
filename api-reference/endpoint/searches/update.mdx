...
<ParamField body="withCoherentPrice" type="boolean">
  Price coherence filter. `true` returns adverts whose price passes our coherence checks; `false` returns adverts flagged as incoherent — both also include adverts whose price has not been assessed. Set to `null` (or send an empty value) to disable filtering. If omitted, defaults to `true`.
</ParamField>
...
<RequestExample>
  ...
  curl --location --request PUT 'https://api.stream.estate/searches/{id}' \
