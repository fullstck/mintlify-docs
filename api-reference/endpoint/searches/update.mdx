...
<ParamField body="withCoherentPrice" type="boolean">
  Price coherence filter. `true` returns only adverts whose price passes our coherence checks; `false` returns only adverts flagged as incoherent. Set to `null` (or omit) to get every result regardless of price coherence.
</ParamField>
